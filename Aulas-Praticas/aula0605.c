 ...
 * $Log: aula0605.c,v $
 * Revision 1.2  2022/01/26 08:02:24  diego.silva
 * *** empty log message ***
 *
 * Revision 1.1  2022/01/25 11:10:22  diego.silva
 * Initial revision
 *
 */
...
int main(int argc, char *argv[])
{
   ...
   codigoRetorno = CalcularMenorComplementar(ordem, linhaElemento, colunaElemento, matrizEntrada, &menorComplementar);

   if (codigoRetorno != ok)
      printf("Erro executando a funcao. Erro numero %u.\n", codigoRetorno);
   else
      printf("\nMenor Complementar do elemento %.5g na linha %u e coluna %u: %.5g\n\n",
             matrizEntrada[linhaElemento - 1][colunaElemento - 1], linhaElemento,
             colunaElemento, menorComplementar);
